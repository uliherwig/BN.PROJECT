...
public class BacktestJobPyDF : IJob
{
    private readonly ILogger<BacktestJobPyDF> _logger;
    ...
    public BacktestJobPyDF(
        ILogger<BacktestJobPyDF> logger,
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        try
        {
            ...
            foreach (var row in data)
            {
                ...
                if (row.index == data.Count && openPosition != null)
                {
                    var posSideMultiplier = openPosition.Side == SideEnum.Buy ? 1 : -1;
                    var currentReturn = (currentPrice - openPosition.PriceOpen) * posSideMultiplier / openPosition.PriceOpen;
                    // calculate overnight holds
